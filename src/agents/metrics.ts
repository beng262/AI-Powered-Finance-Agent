...
export type Series = number[]; // closes, oldest -> newest

export function sma(s: Series, n: number): number | null {
  if (s.length < n || n <= 0) return null;
  const slice = s.slice(-n);
  return slice.reduce((a, b) => a + b, 0) / n;
}

export function ema(s: Series, n: number): number | null {
  if (s.length < n || n <= 0) return null;
  const k = 2 / (n + 1);
...
export function rsi(s: Series, n = 14): number | null {
...
export function momentum(s: Series, n: number): number | null {
  if (s.length < n + 1) return null;
...
export function dailyReturns(s: Series): number[] {
  const r: number[] = [];
  for (let i = 1; i < s.length; i++) if (s[i - 1]) r.push(s[i] / s[i - 1] - 1);
  return r;
}
...
export function annualizedVol(s: Series): number | null {
...
export function historicalVaR(s: Series, conf = 0.95): number | null {
...
export function maxDrawdown(s: Series): number | null {
  if (s.length < 2) return null;
  let peak = s[0], mdd = 0;
  for (const v of s) { if (v > peak) peak = v; const dd = (peak - v) / peak; if (dd > mdd) mdd = dd; }
  return mdd * 100;
}

export const round = (n: number | null, d = 2) => (n == null ? null : Math.round(n * 10 ** d) / 10 ** d);

// --- Indicators ported from shashankvemuri/Finance (deterministic TS versions) ---

// Full EMA series (needed by MACD).
function emaSeries(s: Series, n: number): number[] {
  if (s.length < n) return [];
  const k = 2 / (n + 1);
  const out: number[] = [];
  let e = s.slice(0, n).reduce((a, b) => a + b, 0) / n;
  for (let i = 0; i < s.length; i++) {
    if (i < n) { out.push(e); continue; }
    e = s[i] * k + e * (1 - k);
    out.push(e);
  }
  return out;
...
export function macd(s: Series): { macd: number; signal: number; histogram: number } | null {
...
export function bollinger(s: Series, n = 20, mult = 2): { upper: number; middle: number; lower: number; percentB: number } | null {
...
export function correlation(a: Series, b: Series): number | null {
  ...
  for (let i = 0; i < m; i++) { const u = xa[i] - ma, v = xb[i] - mb; num += u * v; da += u * u; dbv += v * v; }
...
export function smaCrossBacktest(s: Series, fast = 10, slow = 30): {
  strategyPct: number; buyHoldPct: number; trades: number; winRate: number | null; days: number;
} | null {
  if (s.length < slow + 10) return null;
  ...
  for (let i = slow; i < s.length; i++) {
    ...
    const price = s[i];
    if (f > sl && shares === 0) { shares = cash / price; cash = 0; entry = price; trades++; }
    else if (f <= sl && shares > 0) { cash = shares * price; if (price > entry) wins++; shares = 0; }
  }
...
// Deterministic forecast: least-squares linear trend on the series, projected `horizon`
// steps ahead with a confidence band (±1.65σ of residuals, widening with √step ≈ 90%).
// Returns the projected path plus the final-step point estimate and band.
export type Forecast = {
  points: number[]; lower: number[]; upper: number[];
  endEstimate: number; endLow: number; endHigh: number; slopePerStep: number;
};
export function linearForecast(s: Series, horizon: number): Forecast | null {
  const n = s.length;
  if (n < 10 || horizon < 1) return null;
  ...
  // residual stdev
  let ss = 0;
  for (let i = 0; i < n; i++) { const e = s[i] - (a * i + b); ss += e * e; }
  ...
  for (let k = 1; k <= horizon; k++) {
    ...
    points.push(yhat); lower.push(yhat - band); upper.push(yhat + band);
  }
  return {
    points, lower, upper,
