...
export function detectInjection(q: string): { injected: boolean; pattern?: string } {
  for (const re of INJECTION) if (re.test(q)) return { injected: true, pattern: re.source.slice(0, 40) };
...
export function scrubOutput(text: string): string {
  let out = text;
  for (const re of SECRET_PATTERNS) out = out.replace(re, "[gizlendi]");
  return out;
}

// ---- Simple in-memory rate limiter (per session/IP, sliding window) ----
const HITS = new Map<string, number[]>();
export function rateLimit(key: string, limit = 20, windowMs = 60_000): { ok: boolean; retryAfterSec?: number } {
  const now = Date.now();
